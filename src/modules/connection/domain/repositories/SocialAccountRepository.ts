...
import { ISocialAccountRepository } from "./social-account.repository.interface";
...
export class PrismaSocialAccountRepository /* implements ISocialAccountRepository */ {
  ...
  //       pages: {
  //         create:
  //           account.pages?.map((p: SocialPageEntity) => ({
  //             pageId: p.pageId,
  //             name: p.name,
  //             accessToken: p.accessToken,
  //           })) ?? [],
  ...
  //   const pages = await this.prisma.socialPage.findMany({
